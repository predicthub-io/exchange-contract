...
import { ERC1155 } from "openzeppelin-solidity/contracts/token/ERC1155/ERC1155.sol";
import { CTHelpers } from "./CTHelpers.sol";

contract ConditionalTokens is ERC1155 {
    /// @dev Emitted upon the successful preparation of a condition.
    /// @param marketId The condition's ID. This ID may be derived from the other three parameters via ``keccak256(abi.encodePacked(oracle, questionId, outcomeSlotCount))``.
    ...
    function prepareCondition(address oracle, bytes32 questionId, uint outcomeSlotCount) external {
        ...
        bytes32 marketId = CTHelpers.getMarketId(oracle, questionId, outcomeSlotCount);
        require(payoutNumerators[marketId].length == 0, "condition already prepared");
        payoutNumerators[marketId] = new uint[](outcomeSlotCount);
        emit ConditionPreparation(marketId, oracle, questionId, outcomeSlotCount);
    }
    ...
    function reportPayouts(bytes32 questionId, uint[] calldata payouts) external {
        ...
        bytes32 marketId = CTHelpers.getMarketId(msg.sender, questionId, outcomeSlotCount);
        require(payoutNumerators[marketId].length == outcomeSlotCount, "condition not prepared or found");
        require(payoutDenominator[marketId] == 0, "payout denominator already set");
        ...
        for (uint i = 0; i < outcomeSlotCount; i++) {
            ...
            require(payoutNumerators[marketId][i] == 0, "payout numerator already set");
            payoutNumerators[marketId][i] = num;
        }
        require(den > 0, "payout is all zeroes");
        payoutDenominator[marketId] = den;
        emit ConditionResolution(marketId, msg.sender, questionId, outcomeSlotCount, payoutNumerators[marketId]);
    }
    ...
    function splitPosition(
        ...
        for (uint i = 0; i < partition.length; i++) {
            ...
            positionIds[i] = CTHelpers.getPositionId(
                collateralToken,
                CTHelpers.getCollectionId(parentCollectionId, marketId, indexSet)
            );
            amounts[i] = amount;
        }
        ...
        if (freeIndexSet == 0) {
            ...
            _burn(
                ...
                CTHelpers.getPositionId(
                    collateralToken,
                    CTHelpers.getCollectionId(parentCollectionId, marketId, fullIndexSet ^ freeIndexSet)
                ),
                amount
            );
        }
        ...
        emit PositionSplit(msg.sender, collateralToken, parentCollectionId, marketId, partition, amount);
    }
    ...
    function mergePositions(
        ...
        emit PositionsMerge(msg.sender, collateralToken, parentCollectionId, marketId, partition, amount);
    }
    ...
    function redeemPositions(
        ...
        emit PayoutRedemption(msg.sender, collateralToken, parentCollectionId, marketId, indexSets, totalPayout);
    }

    /// @dev Gets the outcome slot count of a condition.
    /// @param marketId ID of the condition.
    /// @return Number of outcome slots associated with a condition, or zero if condition has not been prepared yet.
    function getOutcomeSlotCount(bytes32 marketId) external view returns (uint) {
        return payoutNumerators[marketId].length;
    }
    ...
    /// @param questionId An identifier for the question to be answered by the oracle.
    /// @param outcomeSlotCount The number of outcome slots which should be used for this condition. Must not exceed 256.
    function getMarketId(address oracle, bytes32 questionId, uint outcomeSlotCount) external pure returns (bytes32) {
        return CTHelpers.getMarketId(oracle, questionId, outcomeSlotCount);
    }
    ...
    function getCollectionId(
        ...
        return CTHelpers.getCollectionId(parentCollectionId, marketId, indexSet);
    }
