...
abstract contract Auth is IAuth {
    ...
    /// @dev The set of addresses authorized as Approvers
    ...
    function isApprover(address usr) external view returns (bool) {
        return approvers[usr] == 1;
    }
    ...
    /// Can only be called by a current admin
    /// @param approver_ - The new approver
    function addApprover(address approver_) external onlyAdmin {
        approvers[approver_] = 1;
        emit NewApprover(approver_, msg.sender);
    }
    ...
    /// @notice Removes an existing approver
    /// Can only be called by a current admin
    /// @param approver - The approver to be removed
