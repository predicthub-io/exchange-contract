...
import { PredictHubFactoryHelper } from "./PredictHubFactoryHelper.sol";

/// @title Signatures
/// @notice Maintains logic that defines the various signature types and validates them
abstract contract Signatures is ISignatures, PredictHubFactoryHelper {
    constructor(address _proxyHelper) PredictHubFactoryHelper(_proxyHelper) {}
    ...
    ///                           For signature types BASE_CASTER_PROXY and BASE_CASTER_GNOSIS_SAFE, this is the address of the proxy or the safe
    ...
    function isValidSignature(
        ...
        } else if (signatureType == SignatureType.BASE_CASTER_GNOSIS_SAFE) {
            return verifySafeSignature(signer, associated, structHash, signature);
        } else {
            // BASE_CASTER_PROXY
            return verifyProxySignature(signer, associated, structHash, signature);
        }
    }
    ...
    /// @notice Verifies a signature signed by a PredictHub proxy wallet
    // Verifies that:
    ...
    function verifyProxySignature(
        ...
        return verifyECDSASignature(signer, structHash, signature) && checkProxyWalletAddress(signer, proxyWallet);
    }

    /// @notice Verifies a signature signed by a PredictHub Gnosis safe
    // Verifies that:
    // 1) the ECDSA signature is valid
