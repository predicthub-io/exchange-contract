...
/// @title PredictHubSafeLib
/// @notice Helper library to compute PredictHub gnosis safe addresses
library PredictHubSafeLib {
    bytes private constant _PROXY_CREATION_CODE =
    ...
    /// @notice Gets the PredictHub Gnosis safe address for a signer
    ...
    function getContractBytecode(address masterCopy) internal pure returns (bytes memory) {
        return abi.encodePacked(_PROXY_CREATION_CODE, abi.encode(masterCopy));
    }
