...
interface IRegistryEE {
    ...
    event TokenRegistered(uint256 indexed token0, uint256 indexed token1, bytes32 indexed marketId);
}

abstract contract IRegistry is IRegistryEE {
    function getMarketId(uint256 tokenId) public view virtual returns (bytes32);
