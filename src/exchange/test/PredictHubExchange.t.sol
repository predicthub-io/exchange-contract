...
contract PredictHubExchangeTest is BaseExchangeTest {
    ...
    function testRegisterToken(uint256 _token0, uint256 _token1, uint256 _marketId) public {
        vm.assume(
            _token0 != yes &&
                _token0 != no &&
                _token1 != yes &&
        ...
        bytes32 tokenMarketId = bytes32(_marketId);

        vm.expectEmit(true, true, true, false);
        emit TokenRegistered(_token0, _token1, tokenMarketId);
        emit TokenRegistered(_token1, _token0, tokenMarketId);
        vm.prank(admin);
        exchange.registerToken(_token0, _token1, tokenMarketId);

        assertEq(exchange.getComplement(_token0), _token1);
        assertEq(exchange.getComplement(_token1), _token0);
        assertEq(exchange.getMarketId(_token0), tokenMarketId);
    }
    ...
    function testFillOrders() public {
        ...
        uint256 expectedFeeYesSell = calculateFee(
            100,
            100_000_000,
            yesSell.makerAmount,
            yesSell.takerAmount,
