...
contract MatchOrdersTest is BaseExchangeTest {
    function setUp() public override {
        ...
        _mintTestTokens(anna, address(exchange), 20_000_000_000);
        vm.startPrank(admin);
        dealAndApprove(address(usdc), admin, address(ctf), 20_000_000_000);
        vm.stopPrank();
    }
    ...
    function testMatchTypeMints() public {
        // Init Match with YES buy against a YES sell and a NO buy
        // To match the YES buy with the NO buy, CTF Exchange will MINT new Outcome tokens using it's collateral
        // balance. Then will fill the YES buy and NO buy with the resulting Outcome tokens
        Order memory buy = _createAndSignOrder(bobPK, yes, 60_000_000, 100_000_000, Side.BUY);
        Order memory yesSell1 = _createAndSignOrder(carlaPK, yes, 25_000_000, 12_500_000, Side.SELL);
        Order memory yesSell2 = _createAndSignOrder(annaPK, yes, 25_000_000, 12_500_000, Side.SELL);
        Order memory noBuy = _createAndSignOrder(carlaPK, no, 10_000_000, 25_000_000, Side.BUY);
        Order[] memory makerOrders = new Order[](3);
        makerOrders[0] = yesSell1;
        makerOrders[1] = yesSell2;
        makerOrders[2] = noBuy;

        uint256[] memory fillAmounts = new uint256[](3);
        fillAmounts[0] = 25_000_000;
        fillAmounts[1] = 25_000_000;
        fillAmounts[2] = 10_000_000;

        uint256 takerOrderFillAmount = 40_000_000;
        ...
        // Ensure balances have been updated post match
        assertCTFBalance(bob, yes, 75_000_000);
        assertCollateralBalance(carla, 2_500_000);
        assertCTFBalance(carla, no, 25_000_000);

        // Ensure onchain state for orders is as expected
        // The taker order is partially filled
        assertEq(exchange.getOrderStatus(exchange.hashOrder(buy)).remaining, 20_000_000);
        assertFalse(exchange.getOrderStatus(exchange.hashOrder(buy)).isFilledOrCancelled);

        // The maker orders get completely filled
        assertEq(exchange.getOrderStatus(exchange.hashOrder(yesSell1)).remaining, 0);
        assertTrue(exchange.getOrderStatus(exchange.hashOrder(yesSell1)).isFilledOrCancelled);

        assertEq(exchange.getOrderStatus(exchange.hashOrder(noBuy)).remaining, 0);
        assertTrue(exchange.getOrderStatus(exchange.hashOrder(noBuy)).isFilledOrCancelled);
    }

    function testMatchTypeMintsAbove1() public {
        // Init Match with YES buy against a YES sell and a NO buy
        // To match the YES buy with the NO buy, CTF Exchange will MINT new Outcome tokens using it's collateral
        // balance. Then will fill the YES buy and NO buy with the resulting Outcome tokens
        Order memory buy = _createAndSignOrder(bobPK, yes, 60_000_000, 100_000_000, Side.BUY);
        Order memory yesSell1 = _createAndSignOrder(carlaPK, yes, 25_000_000, 12_500_000, Side.SELL);
        Order memory yesSell2 = _createAndSignOrder(annaPK, yes, 25_000_000, 12_500_000, Side.SELL);
        Order memory noBuy = _createAndSignOrder(carlaPK, no, 10_000_000, 20_000_000, Side.BUY);
        Order[] memory makerOrders = new Order[](3);
        makerOrders[0] = yesSell1;
        makerOrders[1] = yesSell2;
        makerOrders[2] = noBuy;
        ...
        uint256 takerOrderFillAmount = 37_000_000;
        ...
        // Ensure balances have been updated post match
        assertCTFBalance(bob, yes, 70_000_000);
        assertCollateralBalance(carla, 2_500_000);
        assertCTFBalance(carla, no, 20_000_000);

        // Ensure onchain state for orders is as expected
        // The taker order is partially filled
        assertEq(exchange.getOrderStatus(exchange.hashOrder(buy)).remaining, 23_000_000);
        assertFalse(exchange.getOrderStatus(exchange.hashOrder(buy)).isFilledOrCancelled);
    ...
    function testMatchTypeMintAndClaim() public {
        // Init Match with YES buy against a YES sell and a NO buy
        // To match the YES buy with the NO buy, CTF Exchange will MINT new Outcome tokens using it's collateral
        // balance. Then will fill the YES buy and NO buy with the resulting Outcome tokens
        Order memory buy = _createAndSignOrder(bobPK, yes, 60_000_000, 100_000_000, Side.BUY);
        ...
        vm.prank(admin);
        ctf.approveBatch(vm.addr(bobPK), marketId);
        ...
        //assertTruth
        vm.prank(admin);
        bytes32 assertionId = ctf.assertMarket(marketId, yesAnswer);
        uint256 newTime = block.timestamp + 3600;
        vm.warp(newTime);
        vm.prank(admin);
        ...
        partition[1] = 2;
        vm.prank(bob);
        ctf.redeemPositions(marketId, partition);
    }
    ...
    function testMatchTypeComplementaryFuzz(uint128 fillAmount, uint16 takerFeeRateBps, uint16 makerFeeRateBps) public {
        ...
        vm.assume(
            fillAmount <= makerAmount &&
                takerFeeRateBps < exchange.getMaxFeeRate() &&
    ...
    function testWithFees() public {
        ...
        uint256 expectedMakerFee = calculateFee(
            makerFeeRate,
            50_000_000,
            sell.makerAmount,
            sell.takerAmount,
    ...
    function testMergeWithFees() public {
        ...
        uint256 expectedTakerFee = calculateFee(
            takerFeeRate,
            100_000_000,
            yesSell.makerAmount,
            yesSell.takerAmount,
